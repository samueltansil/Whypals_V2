...
import { Trophy, CheckCircle, XCircle, ArrowRight, Volume2, VolumeX, Search } from "lucide-react";
import type { OddOneOutGameConfig } from "@shared/schema";
import { useGameAudio } from "@/hooks/useGameAudio";
import CongratulationsScreen from "./CongratulationsScreen";

interface OddOneOutGameProps {
  config: OddOneOutGameConfig;
...
export default function OddOneOutGame({
  ...
  const handleSelect = (index: number) => {
    ...
    if (index === rounds[current].fakeIndex) {
      playSound("correct");
  ...
  const handleNext = () => {
    ...
    if (current < rounds.length - 1) {
      ...
      setIsComplete(true);
      const finalCorrect = correctCount + (selected === rounds[current].fakeIndex ? 1 : 0);
  ...
  if (isComplete) {
    ...
    return (
      <CongratulationsScreen
        ...
        winMessage={config.winMessage || "Great detective work!"}
        onPlayAgain={restart}
        onBack={onBack}
    ...
    <div className="flex flex-col gap-6 p-4 w-full max-w-xl mx-auto overflow-x-hidden">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="space-y-6"
        >
          <h3 className="text-xl md:text-2xl font-heading font-bold text-center px-2 flex items-center justify-center gap-2">
            <Search className="w-6 h-6 text-primary" /> Which one is NOT true?
          </h3>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="flex-1 text-base md:text-lg">{statement}</span>
                    {answered && isFake && <XCircle className="w-6 h-6 text-red-500 shrink-0" />}
                    {answered && isSelected && !isFake && (
          ...
          {answered && round.explanation && (
            <motion.div
              ...
              className={`p-4 rounded-xl ${selected === round.fakeIndex ? "bg-green-50" : "bg-orange-50"}`}
            >
              <p className="text-sm md:text-base">
                <strong>{selected === round.fakeIndex ? "Spotted it! " : "Not quite. "}</strong>
                {round.explanation}
              </p>
            </motion.div>
          )}
